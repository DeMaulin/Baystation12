	...
	on_reagent_change()
		if (reagents.reagent_list.len > 0)
			...
			switch(reagents.get_master_reagent_id())
				...
				/*if("beer2") //beer2 are in datum/reagents
					icon_state = "beerglass"
					name = "Beer glass"
					desc = "A freezing pint of beer."*/
				if("ale")
	...
	attack(mob/target, mob/user, def_zone)
		...
		if(user.a_intent == I_HURT)
			if(ismob(target) && target.reagents && reagents.total_volume)
				...
				log_attack("\[[time_stamp()]\] <font color='orange'>Has been splashed with [src.name] by [user.name] ([user.ckey]). Reagents: [contained]</font>")
				M.attack_logs_ += text("\[[time_stamp()]\] <font color='orange'>Has been splashed with [src.name] by [user.name] ([user.ckey]). Reagents: [contained]</font>")
				user.attack_logs_ += text("\[[time_stamp()]\] <font color='red'>Used the [src.name] to splash [M.name] ([M.key]). Reagents: [contained]</font>")
		...
		if(icon_state != "glass_empty")
			...
			if(!CanEat(user, target, src, "drink"))//replace with standart_feed_mob?!
				return
